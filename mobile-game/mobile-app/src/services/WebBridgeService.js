...
class WebBridgeService {
  ...
  async init(supabaseConfig) {
    ...
    try {
      ...
      logger.error('Error inicializando WebBridge:', error);
    }
  }
    ...
    try {
      ...
      for (const being of gameState.beings) {
        try {
          ...
          if (error) {
            logger.error(`Error sincronizando being ${being.name}:`, error);
            continue;
          }
  ...
  async pullChangesFromWebapp() {
    try {
      ...
      logger.error('Error en pullChangesFromWebapp:', error);
      return {
        reading_progress: 0,
        achievements: 0,
        beings: 0
  ...
  async updateSyncState() {
    try {
      ...
      logger.error('Error actualizando sync_state:', error);
    }
  }
