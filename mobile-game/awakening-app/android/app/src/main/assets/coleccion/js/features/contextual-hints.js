...
class ContextualHints {
  ...
  evaluateTrigger(trigger) {
    try {
      ...
      const func = new Function(...Object.keys(context), `return ${trigger}`);
  ...
  showHint(hint) {
    ...
    hintElement.innerHTML = `
      <button class="close-btn" onclick="window.contextualHints?.dismissHint('${hint.id}')" title="No mostrar más">×</button>
      <div class="title">
        <span>${hint.title}</span>
      </div>
      <div class="content">${hint.content}</div>
      <div class="actions">
        <button class="got-it-btn" onclick="window.contextualHints?.closeHint()">Entendido</button>
      </div>
    `;
