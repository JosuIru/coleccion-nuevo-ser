...
class OrganismKnowledge {
  ...
  /**
   * Genera conocimiento dinámico basado en los órganos implantados
   */
  generateDynamicKnowledge(beingData) {
    const organs = beingData.organs;
    const categories = [...new Set(organs.map(o => o.category))];
    const books = organs.map(o => o.book);

    // Generar nombre basado en categorías
    const nameTemplates = {
      'Espiritualidad & Ciencia': ['Iluminador', 'Visionario', 'Buscador'],
      'Filosofía Política': ['Reformador', 'Liberador', 'Transformador'],
      'Activismo & Transformación Social': ['Activista', 'Catalizador', 'Revolucionario'],
      'Prácticas & Ejercicios': ['Practicante', 'Maestro', 'Guía'],
      'Prácticas Avanzadas': ['Iniciado', 'Adeptus', 'Trascendente']
    };

    const primaryCategory = categories[0] || 'Espiritualidad & Ciencia';
    const templates = nameTemplates[primaryCategory] || ['Ser'];
    const nameBase = templates[Math.floor(Math.random() * templates.length)];
    const name = `${nameBase} de ${categories.length > 1 ? 'Múltiples Sendas' : primaryCategory.split(' ')[0]}`;

    // Generar propósito basado en órganos
    const purposes = {
      head: 'expandir la consciencia y comprender la realidad última',
      heart: 'cultivar el amor incondicional y la compasión universal',
      leftArm: 'manifestar acciones transformadoras en el mundo',
      rightArm: 'crear y construir nuevas realidades',
      torso: 'integrar cuerpo, mente y espíritu en unidad',
      leftLeg: 'enraizarse en la tierra mientras alcanza el cielo',
      rightLeg: 'caminar el sendero de la transformación'
    };

    const implantedSlots = organs.map(o => o.slot.toLowerCase().replace(/\s+/g, ''));
    const purposeList = Object.entries(purposes)
      .filter(([slot]) => implantedSlots.some(s => s.includes(slot.toLowerCase())))
      .map(([, purpose]) => purpose);

    const purpose = purposeList.length > 0
      ? `Este ser está destinado a ${purposeList.slice(0, 2).join(' y ')}.`
      : 'Explorar los misterios del conocimiento y la transformación personal.';

    // Síntesis basada en libros
    const synthesis = books.length > 1
      ? `Fusiona las enseñanzas de ${books.slice(0, 3).join(', ')}${books.length > 3 ? ' y más' : ''}, creando una síntesis única que trasciende las fronteras de cada disciplina.`
      : `Encarna la sabiduría profunda de "${books[0]}", manifestando sus enseñanzas en cada acción y pensamiento.`;

    // Práctica recomendada
    const practices = [
      'Dedica 10 minutos diarios a la contemplación silenciosa de las enseñanzas integradas.',
      'Practica la escritura reflexiva cada mañana, conectando los conceptos de tus libros.',
      'Realiza una acción consciente diaria inspirada en las enseñanzas de tu ser.',
      'Medita sobre la interconexión de todos los conocimientos que has integrado.',
      'Comparte una enseñanza aprendida con alguien cada día.'
    ];
    const practice = practices[Math.floor(Math.random() * practices.length)];

    return { name, purpose, synthesis, practice, vitality: beingData.vitality, totalOrgans: beingData.totalOrgans };
  }
