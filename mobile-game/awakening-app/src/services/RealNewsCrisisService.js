...
const TRANSLATION_ENABLED = false; // Desactivado temporalmente para debug
...
class RealNewsCrisisService {
  ...
  async getRealWorldCrises(options = {}) {
    ...
    try {
      ...
      if (!crises || crises.length === 0) {
        throw new Error('No se obtuvieron noticias de ninguna fuente RSS');
      }
      ...
      const crisisTemplates = [
        { title: 'Crisis climática en región costera', type: 'environmental', icon: '🌍' },
        { title: 'Protestas por derechos civiles', type: 'social', icon: '✊' },
        { title: 'Emergencia sanitaria declarada', type: 'health', icon: '🏥' },
        { title: 'Tensiones geopolíticas aumentan', type: 'humanitarian', icon: '🕊️' },
        { title: 'Infraestructura crítica afectada', type: 'infrastructure', icon: '🔧' }
      ];

      for (let i = 0; i < Math.min(limit, 10); i++) {
        const template = crisisTemplates[i % crisisTemplates.length];
        proceduralCrises.push({
          id: `procedural_${Date.now()}_${i}`,
          title: `${template.title} #${i + 1}`,
          description: 'Crisis generada para demostración del sistema.',
          source: 'Sistema Procedural',
          type: template.type,
          typeIcon: template.icon,
          typeColor: '#666',
          urgency: 5 + Math.floor(Math.random() * 4),
          scale: 'regional',
          lat: (Math.random() * 140) - 70,
          lon: (Math.random() * 360) - 180,
          locationName: 'Región afectada',
          requiredAttributes: { reflection: 30, empathy: 30, action: 40 },
          rewards: { xp: 100, consciousness: 50 },
  ...
  async fetchViaProxy(url, sourceId) {
    try {
      ...
      // Convertir formato rss2json a nuestro formato
