...
class Biblioteca {
  ...
  async checkIsAdmin(userId) {
    try {
      ...
      if (this._adminCache !== null && now - this._adminCacheTime < this.ADMIN_CACHE_TTL) {
        console.log('[Biblioteca] Usando caché de admin (válido por', Math.floor((this.ADMIN_CACHE_TTL - (now - this._adminCacheTime)) / 1000), 'segundos más)');
      ...
      console.log('[Biblioteca] Caché de admin actualizado:', isAdmin);
  ...
  clearAdminCache() {
    this._adminCache = null;
    this._adminCacheTime = 0;
    console.log('[Biblioteca] Caché de admin limpiado');
  }
  ...
  async handleCosmosNavigationButton(evento) {
    ...
    if (this._loadingCosmos) {
      console.log('[Biblioteca] Cosmos ya se está cargando, ignorando click');
      return;
    }
  ...
  cleanup() {
    console.log('[Biblioteca] Iniciando cleanup...');
    ...
    console.log('[Biblioteca] Cleanup completado');
  }
