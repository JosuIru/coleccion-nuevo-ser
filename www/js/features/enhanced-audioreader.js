...
// 🔧 FIX v3.0.0: Adaptado para usar AudioReader modular existente (window.audioReader)
class EnhancedAudioReader {
  constructor(bookEngine) {
    // 🔧 v3.0.0: Usar AudioReader existente desde window en lugar de crear nueva instancia
    // El AudioReader modular ya está inicializado en /features/audioreader/index.js
    this.baseReader = window.audioReader || null;
    ...
    // Si no existe audioReader aún, esperar a que se inicialice
    if (!this.baseReader) {
      window.addEventListener('audioReaderReady', () => {
        this.baseReader = window.audioReader;
      }, { once: true });
    }
  }
  ...
  async setMode(modeName) {
    ...
    if (mode) {
      ...
      try {
        ...
        logger.error('Error guardando modo de audio:', error);
      }
  ...
  setProfile(profileName) {
    ...
    try {
      ...
      logger.error('Error guardando perfil de audio:', error);
    }
  ...
  loadPreferences() {
    ...
    try {
      ...
      logger.error('Error cargando preferencias de audio:', error);
  ...
  savePreferences() {
    ...
    try {
      ...
      logger.error('Error guardando preferencias de audio:', error);
    }
  }
