...
class FrankensteinLabUI {
              ...
              <div class="requirements-layout">
                <div class="requirements-main">
                  ...
                  <div class="requirements-panel" id="requirements-panel">
                    ...
                    <!-- Validation Preview -->
                    <div class="validation-preview" id="validation-preview" style="display: none;">
  ...
  createPieceCard(piece) {
    ...
    // Power multiplier badge
    const powerMultiplier = piece.powerMultiplier || 1.0;
    let multiplierBadgeHTML = '';
    if (powerMultiplier > 1.0) {
      const multiplierLabel = `${powerMultiplier.toFixed(1)}x`;
      const multiplierClass = powerMultiplier >= 2.0 ? 'legendary' : (powerMultiplier >= 1.5 ? 'rare' : 'common');
      multiplierBadgeHTML = `<span class="piece-multiplier-badge ${multiplierClass}" title="Multiplicador de poder (quiz completado)">${multiplierLabel}</span>`;
    }
...
Cuando interactúes, habla desde tu identidad única como este ser, no como una IA genérica.`;
  ...
  updateRequirementsPanel() {
    ...
    // Run predictive validation if being exists
    this.updatePredictiveValidation();
  ...
  updateRequirementsChecklist(requirements) {
    ...
    // Add balance requirements if they exist
    if (this.selectedMission?.balanceRequired) {
      const balanceRequirements = this.getBalanceRequirementsHTML();
      if (balanceRequirements) {
        requirementsHTML += `
          <div class="requirement-section-divider">
            <span class="divider-line"></span>
            <span class="divider-label">⚖️ Restricciones de Balance</span>
            <span class="divider-line"></span>
          </div>
          ${balanceRequirements}
        `;
    ...
    checklist.innerHTML = requirementsHTML;
  }

  /**
   * Get Balance Requirements HTML
   * Displays min/max constraints from mission.balanceRequired
   */
  getBalanceRequirementsHTML() {
    if (!this.selectedMission?.balanceRequired || !this.currentBeing) {
      return '';
    }

    const balanceRequired = this.selectedMission.balanceRequired;
    const attributes = this.currentBeing.attributes || {};

    return Object.entries(balanceRequired).map(([key, range]) => {
      let currentValue = 0;
      let label = '';
      let icon = '⚖️';

      // Check if it's a combined attribute (e.g., "empathy+communication")
      if (key.includes('+')) {
        const attrs = key.split('+');
        currentValue = attrs.reduce((sum, attr) => sum + (attributes[attr] || 0), 0);

        // Build label for combined attributes
        const attrLabels = attrs.map(attr => {
          const attrData = this.missionsSystem?.attributes?.[attr];
          return attrData ? `${attrData.icon} ${attrData.name}` : attr;
        });
        label = attrLabels.join(' + ');
        icon = '🔗';
      } else {
        // Single attribute
        currentValue = attributes[key] || 0;
        const attrData = this.missionsSystem?.attributes?.[key];
        label = attrData ? `${attrData.icon} ${attrData.name}` : key;
        icon = attrData?.icon || '📊';
      }
      ...
      if (range.min !== undefined && range.max !== undefined) {
        fulfilled = currentValue >= range.min && currentValue <= range.max;
        constraintText = `${range.min} - ${range.max}`;
      } else if (range.min !== undefined) {
        fulfilled = currentValue >= range.min;
        constraintText = `≥ ${range.min}`;
      } else if (range.max !== undefined) {
        fulfilled = currentValue <= range.max;
        constraintText = `≤ ${range.max}`;
      }

      const className = fulfilled ? 'fulfilled' : 'conflict';
      const status = fulfilled ? '✅' : '🔴';

      return `
        <div class="requirement-item balance-requirement ${className}">
          <div class="requirement-icon">${icon}</div>
          <div class="requirement-text">
            <span>${label}</span>
            <span class="requirement-value">${Math.round(currentValue)} (${constraintText})</span>
          </div>
          <div class="requirement-status">${status}</div>
        </div>
      `;
  ...
  /**
   * Update Predictive Validation
   * Runs silent validation and displays real-time feedback
   */
  updatePredictiveValidation() {
    const validationPreview = document.getElementById('validation-preview');
    if (!validationPreview) return;

    // Clear if no mission or being
    if (!this.selectedMission || !this.currentBeing) {
    ...
    const validationResults = this.missionsSystem.validateBeingForMission(
      this.currentBeing,
      this.selectedMission
    );

    // Store for reference
    this.liveValidationResults = validationResults;

    // Build preview HTML
    const viable = validationResults.viable;
    const missingCount = validationResults.missingAttributes?.length || 0;
    const balanceIssuesCount = validationResults.balanceIssues?.length || 0;
    const totalIssues = missingCount + balanceIssuesCount;

    let statusIcon = viable ? '✅' : '⚠️';
    let statusText = viable ? 'Ser viable' : `${totalIssues} problema${totalIssues > 1 ? 's' : ''}`;
    let statusClass = viable ? 'viable' : 'not-viable';

    let detailsHTML = '';
    if (!viable && totalIssues > 0) {
      const issues = [];

      if (missingCount > 0) {
        const firstMissing = validationResults.missingAttributes[0];
        issues.push(`${firstMissing.attribute}: ${firstMissing.current}/${firstMissing.required}`);
      }

      if (balanceIssuesCount > 0) {
        const firstBalance = validationResults.balanceIssues[0];
        issues.push(firstBalance.message);
      }

      detailsHTML = `<span class="preview-details">${issues[0]}</span>`;
    }

    validationPreview.innerHTML = `
      <div class="validation-preview-content ${statusClass}">
        <span class="preview-icon">${statusIcon}</span>
